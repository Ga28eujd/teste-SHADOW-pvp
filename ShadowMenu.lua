...
-- ── Calcula bounding box 2D real iterando todas as partes do char ─
-- Retorna: minX, minY, maxX, maxY em pixels, + bool visible
local function getCharBounds2D(char)
    ...
    for _, part in ipairs(char:GetDescendants()) do
        if part:IsA("BasePart") and part.Name ~= "HumanoidRootPart" then
            -- Projeta os 8 corners do bounding box de cada part
            ...
            for _, corner in ipairs(corners) do
                local sp, onScreen, depth = Camera:WorldToViewportPoint(corner)
...
RunService.RenderStepped:Connect(function()
    ...
    for player, obj in pairs(espObjects) do
        ...
        if not anyEspOn or not hrp or not head or not hum then
            ...
            -- ── Bounding box 2D real ──────────────────────────────
            local minX, minY, maxX, maxY = getCharBounds2D(char)

            if not minX then
                hideAll(obj)
            else
                -- Dimensões do box na tela
                local boxX   = minX
                local boxTop = minY
                local boxW   = maxX - minX
                local boxH   = maxY - minY
                local boxBot = maxY
                local boxCX  = minX + boxW * 0.5  -- centro horizontal

                -- ── Distância e escala ────────────────────────────
                local dist  = (hrp.Position - myPos).Magnitude
                local scale = getScale(dist)

                -- ── Posição 2D da cabeça ──────────────────────────
                local headSP, headOn, headZ = worldToScreen(head.Position)
                ...
                if obj.box then
                    ...
                    if bOn then
                        ...
                        obj.box.Bottom.Position = UDim2.new(0, boxX,                 0, boxBot - thick)
                ...
                if obj.name then
                    ...
                    if nOn then
                        ...
                        obj.name.Size     = UDim2.new(0, math.max(boxW, 80), 0, fs + 4)
                        -- Centralizado acima do topo do box
                        obj.name.Position = UDim2.new(0, boxCX, 0, boxTop - 2)
                    end
                end
                ...
                if obj.rage then
                    ...
                    if rOn then
                        ...
                        obj.rage.Position = UDim2.new(0, boxCX, 0, boxTop - 2 - nameH)
                    end
                end
                ...
                if obj.lifeBG then
                    ...
                    if lOn then
                        ...
                        local ratio = maxHp > 0
                            and math.clamp(hum.Health / maxHp, 0, 1) or 0
