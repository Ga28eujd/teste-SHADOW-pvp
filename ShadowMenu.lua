...
-- ══════════════════════════════════════════════════════════════════
-- Projeta posições 3D das partes do corpo na tela
-- Âncoras: Head (topo), HumanoidRootPart (centro), LeftFoot/RightFoot
-- ou fallback HRP-3 studs. Confiável em qualquer distância.
-- ══════════════════════════════════════════════════════════════════
local function getBodyAnchors(char, hrp, head)
    -- Topo: ponto acima da cabeça (Head.Position + Head.Size.Y/2)
    local headTop = head.Position + Vector3.new(0, head.Size.Y / 2, 0)

    -- Pés: tenta encontrar LeftFoot ou HumanoidRootPart - metade altura
    local feetPart = char:FindFirstChild("LeftFoot")
    ...
    local feetPos
    if feetPart then
        feetPos = feetPart.Position - Vector3.new(0, feetPart.Size.Y / 2, 0)
    else
        -- Fallback: HRP menos metade da altura estimada do personagem
        local hum = char:FindFirstChildOfClass("Humanoid")
        local hipH = hum and hum.HipHeight or 1.35
        feetPos = hrp.Position - Vector3.new(0, hipH + 1.0, 0)
    end

    -- Centro horizontal: usa HRP como âncora X/Z mais estável
    local centerPos = Vector3.new(hrp.Position.X, headTop.Y, hrp.Position.Z)

    return headTop, feetPos, head.Position
end
...
RunService.RenderStepped:Connect(function()
    ...
    for player, obj in pairs(espObjects) do
        ...
        -- ── Projeta âncoras do corpo ──────────────────────────────
        local headTopW, feetPosW, headCenterW = getBodyAnchors(char, hrp, head)

        local topSP,    topOn,    topZ    = Camera:WorldToViewportPoint(headTopW)
        local feetSP,   feetOn,   feetZ   = Camera:WorldToViewportPoint(feetPosW)
        local headSP3,  headOn,   headZ   = Camera:WorldToViewportPoint(headCenterW)

        -- Só renderiza se o topo OU os pés estiverem na frente da câmera
        if (topZ <= 0 and feetZ <= 0) or not (topOn or feetOn or headOn) then
            hideAll(obj)
            continue
        end

        local headSP  = Vector2.new(headSP3.X, headSP3.Y)
        local topSP2  = Vector2.new(topSP.X,   topSP.Y)
        local feetSP2 = Vector2.new(feetSP.X,  feetSP.Y)

        -- ── Dimensões 2D derivadas dos pontos do corpo ────────────
        -- Altura = da cabeça (topo) até os pés
        local boxTop = math.min(topSP2.Y,  headSP.Y)
        local boxBot = feetSP2.Y
        local boxH   = math.max(boxBot - boxTop, 1)
        -- Largura proporcional à altura (proporção R6/R15 ≈ 0.45)
        local boxW   = math.max(boxH * 0.45, 10)
        -- Centro X = HRP projetado (mais estável que cabeça)
        local hrpSP  = Camera:WorldToViewportPoint(hrp.Position)
        local boxCX  = hrpSP.X
        local boxX   = boxCX - boxW / 2
        ...
        if obj.name then
            ...
            if nOn then
                ...
                obj.name.Size     = UDim2.new(0, math.max(boxW + 20, 80), 0, fs + 4)
                -- Acima do topo do box, centralizado no X do HRP
                obj.name.Position = UDim2.new(0, boxCX, 0, boxTop - fs - 4)
            end
        end
        ...
        if obj.rage then
            ...
            if rOn then
                ...
                obj.rage.Size     = UDim2.new(0, math.max(boxW, 60), 0, fs + 4)
                -- Acima do NAME (se ativo) ou acima do box
                local nameOffset = state.toggles.ESP_NAME
                    and (math.max(8, math.floor(14 * scale)) + 4) or 0
                obj.rage.Position = UDim2.new(0, boxCX, 0, boxTop - fs - 4 - nameOffset)
            end
        end
        ...
        if obj.trace then
            ...
            if tOn then
                local topScreen = Vector2.new(Camera.ViewportSize.X / 2, 0)
                drawLine2D(obj.trace, topScreen, headSP, thick)
        ...
        if obj.lifeBG then
            ...
            if lOn then
                ...
                obj.lifeBG.Position = UDim2.new(0, boxX + boxW + barGap, 0, boxTop)
