...
-- ══════════════════════════════════════════════════════════════════
--  SISTEMA DE ESCALA DOS ESP
--  Ajuste os valores abaixo para controlar o tamanho dos ESP
-- ══════════════════════════════════════════════════════════════════

-- Distâncias de referência (em studs)
local DIST_PERTO  = 10    -- abaixo disso → tamanho MÁXIMO
local DIST_LONGE  = 400   -- acima disso  → tamanho MÍNIMO

-- Fator de escala (1.0 = tamanho base, 0.0 = invisível)
local ESCALA_MAX  = 1.0   -- tamanho quando MT PERTO
local ESCALA_MIN  = 0.25  -- tamanho quando MT LONGE

-- Tamanhos BASE dos BillboardGui (em pixels, na distância PERTO)
local BB_NAME_W   = 120   -- largura  do NAME
local BB_NAME_H   = 20    -- altura   do NAME
local BB_RAGE_W   = 100   -- largura  do RAGE
local BB_RAGE_H   = 16    -- altura   do RAGE
local BB_BOX_W    = 80    -- largura  do BOX
local BB_BOX_H    = 120   -- altura   do BOX
local BB_LIFE_W   = 8     -- largura  do LIFE
local BB_LIFE_H   = 120   -- altura   do LIFE
local BB_HEAD_SZ  = 30    -- tamanho  do HEAD (círculo)

-- Offsets verticais BASE dos BillboardGui (StudsOffset)
local OFF_NAME_Y  = 2.2   -- altura acima da cabeça para o NAME
local OFF_RAGE_Y  = 3.0   -- altura acima da cabeça para o RAGE

-- ── Calcula fator de escala baseado na distância ─────────────────
...
-- Gera nome aleatório para evitar detecção por nome
...
-- ── Cria BillboardGui no PlayerGui (não no personagem inimigo) ───
-- Adornee aponta para a parte do corpo mas o objeto fica no nosso PlayerGui
local function makeBillboard(adornee, studOffset, sizeX, sizeY)
    local bb = Instance.new("BillboardGui")
    bb.Name           = randName()   -- nome aleatório, não detectável
    bb.Adornee        = adornee
    bb.Size           = UDim2.new(0, sizeX, 0, sizeY)
    bb.StudsOffset    = studOffset
    bb.AlwaysOnTop    = true
    bb.LightInfluence = 0
    bb.ResetOnSpawn   = false
    bb.Parent         = PlayerGui  -- fica no nosso PlayerGui, não no inimigo
    return bb
end
...
local function createEspForPlayer(plr)
    ...
    local nameBB = makeBillboard(head, Vector3.new(0, 2.2, 0), 120, 20)
    local nameLbl = Instance.new("TextLabel")
    nameLbl.Name                   = randName()
    ...
    -- ── ESP RAGE — acima do NAME ──────────────────────────────────
    local rageBB = makeBillboard(head, Vector3.new(0, 3.0, 0), 100, 16)
    local rageLbl = Instance.new("TextLabel")
    rageLbl.Name                   = randName()
    ...
    -- ── ESP BOX — 4 frames dentro de BillboardGui no HRP ─────────
    -- SizeInStuds = escala automaticamente com distância da câmera
    local boxBB = makeBillboard(hrp, Vector3.new(0, 1.0, 0), 80, 120)
    boxBB.SizeInStuds = Vector2.new(2.2, 5.5)
    boxBB.Size        = UDim2.new(1, 0, 1, 0)
    local espCol = getEspColor()
    local box = {}
    for _, side in ipairs({"Top","Bottom","Left","Right"}) do
        local f = Instance.new("Frame")
        f.Name             = randName()
        f.BackgroundColor3 = espCol
        f.BorderSizePixel  = 0
        f.Visible          = false
        f.Parent           = boxBB
        box[side] = f
    ...
    -- ── ESP TRACE — Frame no EspContainer (linha topo→cabeça) ────
    local traceLine = Instance.new("Frame")
    traceLine.Name             = randName()
    traceLine.BackgroundColor3 = espCol
    ...
    -- ── ESP LIFE — BillboardGui no HRP, barra lateral ────────────
    -- Offset lateral para ficar à direita do box (1.4 studs = metade box + gap)
    local lifeBB = makeBillboard(hrp, Vector3.new(1.4, 1.0, 0), 8, 120)
    lifeBB.SizeInStuds = Vector2.new(0.3, 5.5)
    lifeBB.Size        = UDim2.new(1, 0, 1, 0)
    local lifeBG = Instance.new("Frame")
    lifeBG.Name             = randName()
    lifeBG.Size             = UDim2.new(1, 0, 1, 0)
    lifeBG.BackgroundColor3 = Color3.fromRGB(40, 40, 40)
    lifeBG.BorderSizePixel  = 0
    lifeBG.Visible          = false
    lifeBG.Parent           = lifeBB
    local lifeBar = Instance.new("Frame")
    lifeBar.Name             = randName()
    lifeBar.Size             = UDim2.new(1, 0, 1, 0)
    lifeBar.AnchorPoint      = Vector2.new(0, 1)
    lifeBar.Position         = UDim2.new(0, 0, 1, 0)
    ...
    -- ── ESP HEAD — círculo semiopaco na cabeça ────────────────────
    local headBB = makeBillboard(head, Vector3.new(0, 0, 0), 30, 30)
    headBB.SizeInStuds = Vector2.new(1.2, 1.2)
    headBB.Size        = UDim2.new(1, 0, 1, 0)
    local headCircle = Instance.new("Frame")
    headCircle.Name                   = randName()
    headCircle.Size                   = UDim2.new(1, 0, 1, 0)
    ...
    headCircle.Parent                 = headBB
    local hCorner = Instance.new("UICorner")
    hCorner.CornerRadius = UDim.new(1, 0)
    hCorner.Parent       = headCircle
    obj.headBB     = headBB
...
local function removeEspForPlayer(plr)
    ...
    pcall(function() obj.boxBB:Destroy()   end)
    pcall(function() obj.trace:Destroy()   end)
    pcall(function() obj.lifeBB:Destroy()  end)
    pcall(function() obj.headBB:Destroy()  end)
    espObjects[plr] = nil
end

-- ── Linha 2D (só usada pelo TRACE) ───────────────────────────────
    ...
    for plr, obj in pairs(espObjects) do
        pcall(function()
            ...
            if obj.nameLbl.Visible then
                -- Escala o BillboardGui inteiro: quanto mais longe, menor
                local w = math.max(40, math.floor(BB_NAME_W * scale))
                local h = math.max(8,  math.floor(BB_NAME_H * scale))
                obj.nameBB.Size         = UDim2.new(0, w, 0, h)
                obj.nameBB.StudsOffset  = Vector3.new(0, OFF_NAME_Y * scale, 0)
                obj.nameLbl.TextColor3  = espCol
            end

            -- ── ESP RAGE ──────────────────────────────────────────
            obj.rageLbl.Visible = state.toggles.ESP_RAGE
            if obj.rageLbl.Visible then
                local w = math.max(30, math.floor(BB_RAGE_W * scale))
                local h = math.max(6,  math.floor(BB_RAGE_H * scale))
                obj.rageBB.Size         = UDim2.new(0, w, 0, h)
                obj.rageBB.StudsOffset  = Vector3.new(0, OFF_RAGE_Y * scale, 0)
                obj.rageLbl.Text        = math.floor(dist) .. "m"
            end

            -- ── ESP BOX ───────────────────────────────────────────
            -- SizeInStuds = tamanho em unidades do mundo 3D
            -- O Roblox escala automaticamente com a distância da câmera
            ...
            if bOn then
                -- Tamanho em studs: personagem ~2 studs largo, ~5 studs alto
                obj.boxBB.SizeInStuds = Vector2.new(2.2, 5.5)
                obj.boxBB.Size        = UDim2.new(1, 0, 1, 0)

                local thick = math.max(1, math.floor(2 * scale))
                obj.box.Top.Position    = UDim2.new(0, 0,      0, 0)
                obj.box.Top.Size        = UDim2.new(1, 0,      0, thick)
                obj.box.Bottom.Position = UDim2.new(0, 0,      1, -thick)
                obj.box.Bottom.Size     = UDim2.new(1, 0,      0, thick)
                obj.box.Left.Position   = UDim2.new(0, 0,      0, 0)
                obj.box.Left.Size       = UDim2.new(0, thick,  1, 0)
                obj.box.Right.Position  = UDim2.new(1, -thick, 0, 0)
                obj.box.Right.Size      = UDim2.new(0, thick,  1, 0)
            end

            -- ── ESP TRACE ─────────────────────────────────────────
            -- Pega o topo real do personagem na tela (não a cabeça)
            obj.trace.Visible = state.toggles.ESP_TRACE
            if obj.trace.Visible then
                local topPos3D = head.Position + Vector3.new(0, 3.0, 0)
                local topSP    = Camera:WorldToViewportPoint(topPos3D)
                local headSP   = Camera:WorldToViewportPoint(head.Position)
                if headSP.Z > 0 then
                    local topV   = Vector2.new(topSP.X, topSP.Y)
                    local thick  = math.max(1, math.floor(2 * scale))
                    drawLine2D(obj.trace, Vector2.new(Camera.ViewportSize.X / 2, 0), topV, thick)
            ...
            -- ── ESP LIFE ──────────────────────────────────────────
            obj.lifeBG.Visible = state.toggles.ESP_LIFE
            if obj.lifeBG.Visible then
                local maxHp = hum.MaxHealth
                local ratio = maxHp > 0 and math.clamp(hum.Health / maxHp, 0, 1) or 0
            ...
            if obj.headCircle.Visible then
                ...
                obj.headCircle.BackgroundColor3 = los
