...
RunService.RenderStepped:Connect(function()
    ...
    for plr, obj in pairs(espObjects) do
        pcall(function()
            ...
            -- ── ESP NAME ──────────────────────────────────────────
            -- BillboardGui já está ancorado na cabeça — só liga/desliga
            obj.nameLbl.Visible = state.toggles.ESP_NAME
            if obj.nameLbl.Visible then
                local fs = math.clamp(math.floor(14 * scale), 8, 14)
                obj.nameLbl.TextSize = fs
            ...
            if obj.rageLbl.Visible then
                local fs = math.clamp(math.floor(12 * scale), 7, 12)
                obj.rageLbl.TextSize = fs
                obj.rageLbl.Text     = math.floor(dist) .. "m"
            end

            -- ── ESP BOX ───────────────────────────────────────────
            -- BillboardGui no HRP — tamanho em Studs se adapta automaticamente
            ...
            -- ── ESP TRACE ─────────────────────────────────────────
            -- Trace ainda usa worldToScreen pois cruza a tela inteira
            ...
            -- ── ESP LIFE ──────────────────────────────────────────
            -- BillboardGui no HRP, posicionado ao lado direito do box
            ...
            -- ── ESP HEAD ──────────────────────────────────────────
            -- BillboardGui na cabeça — círculo semiopaco
