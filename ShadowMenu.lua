...
local function createEspForPlayer(plr)
    ...
    local boxBB = makeBillboard(hrp, Vector3.new(0, 0, 0), 80, 120)
    ...
    local lifeBB = makeBillboard(hrp, Vector3.new(0, 0, 0), 8, 120)
...
RunService.RenderStepped:Connect(function()
    ...
    for plr, obj in pairs(espObjects) do
        pcall(function()
            ...
            if bOn then
                -- Escala o BillboardGui do box
                local w = math.max(20, math.floor(BB_BOX_W * scale))
                local h = math.max(30, math.floor(BB_BOX_H * scale))
                obj.boxBB.Size = UDim2.new(0, w, 0, h)
            ...
            if obj.trace.Visible then
                ...
                local headV  = Vector2.new(headSP.X, headSP.Y)
                if headSP.Z > 0 then
                    local thick = math.max(1, math.floor(2 * scale))
                    drawLine2D(obj.trace, Vector2.new(Camera.ViewportSize.X / 2, 0), headV, thick)
            ...
            if obj.lifeBG.Visible then
                -- Escala a largura e altura da barra de vida
                local w = math.max(3, math.floor(BB_LIFE_W * scale))
                local h = math.max(20, math.floor(BB_LIFE_H * scale))
                obj.lifeBB.Size = UDim2.new(0, w, 0, h)
            ...
            if obj.headCircle.Visible then
                -- Escala o círculo da cabeça
                local sz = math.max(8, math.floor(BB_HEAD_SZ * scale))
                obj.headBB.Size = UDim2.new(0, sz, 0, sz)
