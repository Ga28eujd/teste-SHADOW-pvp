...
local function createEspForPlayer(plr)
    ...
    -- HEAD — Highlight na cabeça (sempre tamanho real, sem deriva)
    local headHL = Instance.new("Highlight")
    headHL.FillColor           = Color3.fromRGB(255, 0, 0)
    headHL.OutlineTransparency = 1
    headHL.DepthMode           = Enum.HighlightDepthMode.AlwaysOnTop
    headHL.Adornee             = head
    headHL.Parent              = head
    headHL.Enabled             = false
    obj.headHL = headHL
...
local function removeEspForPlayer(plr)
    ...
    pcall(function() obj.headHL:Destroy()   end)
    espObjects[plr] = nil
end
...
RunService.RenderStepped:Connect(function()
    ...
    for plr, obj in pairs(espObjects) do
        pcall(function()
            ...
            -- HEAD — Highlight na cabeça (tamanho real, sem deriva)
            obj.headHL.Enabled = state.toggles.ESP_HEAD
        end)
    end
end)
