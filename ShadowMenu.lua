...
-- VERSÃO CORRIGIDA: FOV em GRAUS + SHOWFOV aparecendo quando ativado
...
-- getBestTarget CORRIGIDO: FOV em GRAUS, não em pixels
local function getBestTarget()
    ...
    -- CORRIGIDO: Converte FOV de graus para pixels na tela
    local fovPixels    = math.tan(math.rad(state.fov / 2)) * cam.ViewportSize.X
    ...
    for _, plr in ipairs(Players:GetPlayers()) do
        ...
        -- CORRIGIDO: Compara com fovPixels (não com distância)
        local dist2D = (Vector2.new(sp.X, sp.Y) - screenCenter).Magnitude
        if dist2D < fovPixels and dist2D < bestDist then
            bestDist = dist2D
            bestChar = char
        end
    end
...
RunService.RenderStepped:Connect(function()
    ...
    -- FOV Circle: CORRIGIDO - Converte FOV em graus para pixels
    local fovPixels = math.tan(math.rad(state.fov / 2)) * cam.ViewportSize.X
    fovCircle.Visible  = state.toggles.SHOWFOV
    fovCircle.Position = Vector2.new(cam.ViewportSize.X/2, cam.ViewportSize.Y/2)
    fovCircle.Radius   = fovPixels
...
print("[ShadowMenu] Carregado! Pressione INSERT para abrir o menu.")
print("[✓] AIMLOCK | [✓] WALLCK | [✓] SHOWFOV CORRIGIDO | [✓] FOV CONFIG CORRIGIDO")
