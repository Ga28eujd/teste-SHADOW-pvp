...
local function createEspForPlayer(plr)
    ...
    -- TRACE — Frame rotacionado (topo tela → topo personagem)
    local trace = Instance.new("Frame")
    trace.Name = randName()
    trace.BackgroundColor3 = col
    trace.BorderSizePixel = 0
    trace.Visible = false
    trace.Parent = EspContainer
    obj.trace = trace

    -- LIFE — BillboardGui no HRP, barra ao lado direito
    local lifeBB = makeBB(hrp, Vector3.new(0, 0, 0), BB_LIFE_W, BB_LIFE_H)
    ...
    -- HEAD — BillboardGui na cabeça, círculo semiopaco
    local headBB = makeBB(head, Vector3.new(0,0,0), BB_HEAD_SZ, BB_HEAD_SZ)
...
RunService.RenderStepped:Connect(function()
    ...
    for plr, obj in pairs(espObjects) do
        pcall(function()
            ...
            -- TRACE — linha do centro-topo da tela até a cabeça do inimigo
            obj.trace.Visible = state.toggles.ESP_TRACE
            if obj.trace.Visible then
                -- Ponto final = centro exato da cabeça na tela
                ...
                if headSP.Z > 0 then
                    ...
                    local to    = Vector2.new(headSP.X, headSP.Y)
                    drawLine2D(obj.trace, from, to, thick)
                    obj.trace.BackgroundColor3 = col
                else
                    obj.trace.Visible = false
                end
            end

            -- LIFE — barra lateral direita alinhada ao BOX
            obj.lifeBG.Visible = state.toggles.ESP_LIFE
            if obj.lifeBG.Visible then
                -- Calcula a altura do box em pixels para alinhar a barra
                local topSP    = Camera:WorldToViewportPoint(head.Position + Vector3.new(0, head.Size.Y/2, 0))
                local bottomSP = Camera:WorldToViewportPoint(hrp.Position  - Vector3.new(0, hrp.Size.Y/2, 0))
                local rightSP  = Camera:WorldToViewportPoint(hrp.Position + hrp.CFrame.RightVector * (hrp.Size.X/2))
                local centerSP = Camera:WorldToViewportPoint(hrp.Position)

                if topSP.Z > 0 then
                    local boxH   = math.abs(bottomSP.Y - topSP.Y)
                    local boxTop = math.min(topSP.Y, bottomSP.Y)
                    local barW   = math.max(3, math.floor(4 * scale))
                    -- X da barra = lado direito do box + pequeno gap
                    local barX   = math.max(rightSP.X, centerSP.X) + 3

                    obj.lifeBB.StudsOffset = Vector3.new(hrp.Size.X/2 + 0.4, 0, 0)
                    -- Tamanho em pixels proporcional à altura real do box na tela
                    obj.lifeBB.Size = UDim2.new(0, barW, 0, math.max(10, math.floor(boxH)))

                    local maxHp = hum.MaxHealth
                    local ratio = maxHp > 0 and math.clamp(hum.Health/maxHp, 0, 1) or 0
                    -- Barra cresce de baixo para cima
            ...
            -- HEAD — tamanho real da cabeça na tela, com sistema de distância
            obj.headCircle.Visible = state.toggles.ESP_HEAD
            if obj.headCircle.Visible then
                -- Calcula o tamanho da cabeça em pixels usando 2 pontos projetados
                local headCenter = Camera:WorldToViewportPoint(head.Position)
                local headEdge   = Camera:WorldToViewportPoint(head.Position + Vector3.new(head.Size.X/2, 0, 0))
                if headCenter.Z > 0 then
                    -- Raio em pixels = distância do centro à borda da cabeça
                    local radius = math.max(6, math.abs(headEdge.X - headCenter.X))
                    local sz     = math.floor(radius * 2)
