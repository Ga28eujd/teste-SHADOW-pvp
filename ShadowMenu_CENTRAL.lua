...
-- ══════════════════════════════════════════════════════════════════
-- AIMBOT LOGIC — extraído do aimlock funcional
-- ══════════════════════════════════════════════════════════════════

-- Lista de nomes de armas reconhecidos (case-insensitive)
...
-- FOV Circle — Drawing, sempre existe
...
-- Retorna o alvo mais próximo do centro da tela dentro do FOV
-- Respeita WALLCK (ignora alvos atrás de parede) e FIRELOCK (parte do corpo)
local function getBestTarget()
    ...
    for _, plr in ipairs(Players:GetPlayers()) do
        ...
        -- Só mira em inimigos (time diferente)
        ...
        -- WALLCK: verifica se tem parede entre meu HRP e o alvo
...
game:GetService("RunService").RenderStepped:Connect(function()
    ...
    -- FOV Circle: sempre atualiza, visível só quando SHOWFOV ligado
    local fovRadius = (state.fov / 180) * (cam.ViewportSize.X / 2)
    ...
    if state.toggles.AIMLOCK then
        ...
        if targetChar then
            -- Parte do corpo alvo definida pelo FIRELOCK
            ...
            if targetPart then
                ...
                -- 2. Corpo do player olha para o alvo (só horizontal)
                local myChar = LocalPlayer.Character
                local myHRP  = myChar and myChar:FindFirstChild("HumanoidRootPart")
                if myHRP then
