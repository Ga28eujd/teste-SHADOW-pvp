...
local function createEspForPlayer(plr)
    ...
    -- ── ESP BOX — Drawing.Quad com 4 pontos 3D reais ─────────────
    -- Mesma lógica do ESP funcional: GPU-rendered, nunca deriva
    local box = Drawing.new("Quad")
    box.Visible      = false
    box.Thickness    = 1.5
    box.Transparency = 1
    box.Filled       = false
    box.Color        = getEspColor()
    obj.box = box
...
RunService.RenderStepped:Connect(function()
    ...
    for plr, obj in pairs(espObjects) do
        pcall(function()
            ...
            -- ── ESP BOX — Drawing.Quad, lógica exata do funcional ────
            ...
            if not bOn then
                ...
                local c1 = Camera:WorldToViewportPoint(top    + right + front)
                local c2 = Camera:WorldToViewportPoint(top    - right + front)
                local c3 = Camera:WorldToViewportPoint(bottom - right - front)
                local c4 = Camera:WorldToViewportPoint(bottom + right - front)
